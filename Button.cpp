...
void Button::unshrink() {
	//Restore old scale and position of buttons after redirection of mouse
	this->sprite.setScale(sf::Vector2f(NORMAL_MULTIPLIER, NORMAL_MULTIPLIER));
	this->sprite.setPosition(this->pos);
}

//Constructor / Destructor
Button::Button(const sf::Texture* texture, const sf::Vector2f& pos, InterfaceBlock name) : InterfaceComponent(texture, pos, name) {}
